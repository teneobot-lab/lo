...
export const RejectManager: React.FC<RejectManagerProps> = ({
  ...
  const exportFlattenedExcel = () => {
    ...
    const wscols = [ {wch: 15}, {wch: 45}, {wch: 10}, ...uniqueDates.map(() => ({wch: 14})), {wch: 18} ];
    ws['!cols'] = wscols;
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-2xl animate-in fade-in duration-300">
            <div className="bg-white dark:bg-gray-900 rounded-[3.5rem] shadow-[0_40px_160px_-16px_rgba(0,0,0,0.6)] w-full max-w-7xl max-h-[95vh] overflow-hidden flex flex-col border border-white/20 animate-in zoom-in duration-300">
                {/* Modal Header */}
                <div className="px-12 py-8 border-b border-slate-100 dark:border-gray-800 flex justify-between items-center bg-gradient-to-r from-rose-50 to-white dark:from-gray-800 dark:to-gray-900">
                    <div className="flex items-center gap-6">
                        <div className="p-4 bg-rose-500 rounded-[2rem] text-white shadow-2xl shadow-rose-500/30">
                            <AlertTriangle size={32} />
                        </div>
                        <div>
                            <h3 className="text-3xl font-black text-slate-800 dark:text-white uppercase tracking-tighter leading-none mb-1">{log ? 'Update Laporan Reject' : 'Catat Barang Reject'}</h3>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.4em]">Sistem Manajemen Kerusakan & Pemusnahan Aset</p>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-10 space-y-8">
                    {/* SECTION 1: Detail Informasi (Atas) */}
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[2.5rem] shadow-paper border border-slate-100 dark:border-gray-700 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none"><FileText size={120} /></div>
                        <h4 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-[0.4em] mb-8 border-b border-slate-50 dark:border-gray-700 pb-3">I. Informasi Laporan Umum</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    ...
                                    <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full pl-16 pr-6 py-6 bg-slate-50 dark:bg-gray-900 border-2 border-transparent focus:border-rose-400 rounded-[2.5rem] outline-none font-bold text-xl text-slate-700 dark:text-white dark:[color-scheme:dark] shadow-inner" />
                                </div>
                            </div>
                            <div className="space-y-3">
                                <label className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] block ml-3">Keterangan Laporan</label>
                                <div className="relative">
                                    <Clipboard className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400" size={20}/>
                                    <input value={notes} onChange={e => setNotes(e.target.value)} className="w-full pl-16 pr-6 py-6 bg-slate-50 dark:bg-gray-900 border-2 border-transparent focus:border-rose-400 rounded-[2.5rem] outline-none font-bold text-xl text-slate-700 dark:text-white shadow-inner" placeholder="Contoh: Kerusakan saat distribusi..." />
                                </div>
                            </div>
                        </div>
                    </div>

                    {/* SECTION 2: Input Barang (Tengah) */}
                    <div className="p-10 bg-slate-50 dark:bg-gray-800/50 rounded-[3rem] border border-slate-200 dark:border-gray-700 space-y-8 relative overflow-hidden shadow-2xl">
                        <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none"><Zap size={140} /></div>
                        <h4 className="text-xs font-black text-rose-500 uppercase tracking-[0.4em] flex items-center gap-3"><Zap size={18} fill="currentColor"/> II. Detail Input Barang Reject</h4>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-center">
                            {selectedMaster && qty && (
                                <div className="flex-1 flex items-center gap-4 p-6 bg-rose-50/70 dark:bg-rose-900/10 rounded-[2rem] border border-rose-100 dark:border-rose-900/30 animate-in slide-in-from-top-4">
                                    <TrendingDown size={28} className="text-rose-500" />
                                    <div>
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Impact Stock Dasar</p>
                            ...
                            <button onClick={handleAddItem} disabled={!selectedMasterId || !qty || !reason} className="flex-[1.5] py-6 bg-rose-500 hover:bg-rose-600 text-white rounded-[2.5rem] font-black shadow-2xl shadow-rose-500/30 flex items-center justify-center gap-4 transition-all active:scale-[0.98] disabled:opacity-30 uppercase tracking-[0.2em] text-sm">
                                <Plus size={24}/> Masukkan Ke Keranjang (Enter)
                            </button>
                        </div>
                    </div>

                    {/* SECTION 3: Daftar Tunggu / Keranjang (Bawah) */}
                    <div className="bg-white dark:bg-gray-800 p-10 rounded-[2.5rem] shadow-paper border border-slate-100 dark:border-gray-700 flex flex-col min-h-[400px]">
                        <div className="flex justify-between items-center border-b-2 border-slate-50 dark:border-gray-700 pb-6 mb-8">
                            <h4 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-[0.4em] flex items-center gap-4"><ShoppingCart size={24}/> III. Daftar Tunggu Konfirmasi</h4>
                            <span className="bg-rose-500 text-white px-4 py-2 rounded-2xl text-[10px] font-black shadow-lg">{items.length} RECORD</span>
                        </div>
                        
                        <div className="flex-1 overflow-auto rounded-[2rem] border border-slate-50 dark:border-gray-700">
                             <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-gray-900 border-b border-slate-100 dark:border-gray-700 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    <tr>
                                        <th className="p-6">Barang & SKU</th>
                                        <th className="p-6 text-center">Qty Input</th>
                                        <th className="p-6 text-center">Impact Stok Dasar</th>
                                ...
                                <tbody className="divide-y divide-slate-50 dark:divide-gray-800">
                                    {items.map((it, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-gray-700/50 transition-colors">
                                            <td className="p-6">
                                                ...
                                                <div className="text-[10px] font-mono text-slate-400 tracking-widest">{it.sku}</div>
                                            </td>
                                            <td className="p-6 text-center">
                                                <div className="text-sm font-black text-rose-500 uppercase tracking-widest">{it.quantity} {it.unit}</div>
                                            </td>
                                            <td className="p-6 text-center">
                                                <div className="text-sm font-bold text-paper-blue uppercase tracking-tighter">{it.totalBaseQuantity.toFixed(3)} {it.baseUnit}</div>
                                            </td>
                                            <td className="p-6">
                                                <div className="text-[11px] font-bold text-slate-500 uppercase bg-slate-100 dark:bg-gray-800 px-3 py-1 rounded-full inline-block">{it.reason}</div>
                                            </td>
                                            <td className="p-6 text-right">
                                        ...
                                        <tr>
                                            <td colSpan={5} className="p-24 text-center">
                                                <div className="flex flex-col items-center justify-center space-y-6 opacity-20">
                                                    ...
                                                    <p className="text-sm font-black uppercase tracking-[0.5em]">Belum Ada Barang Di Keranjang</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                ...
                {/* MODAL FOOTER */}
                <div className="p-10 border-t border-slate-50 dark:border-gray-800 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50 dark:bg-gray-800">
                    <div className="flex items-center gap-6">
                        <div className="p-5 bg-slate-900 rounded-[1.5rem] text-white shadow-2xl">
                             <p className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em] mb-1">Akumulasi Item</p>
                             <span className="text-3xl font-black tracking-tighter leading-none">{items.length} <span className="text-xs text-white/30 uppercase font-black ml-1 tracking-widest">RECORD</span></span>
                        </div>
                    </div>
                    <div className="flex gap-4 w-full md:w-auto">
                        <button onClick={onClose} className="flex-1 md:flex-none px-10 py-5 text-slate-400 font-black hover:bg-white dark:hover:bg-gray-700 rounded-[2rem] uppercase tracking-widest text-[11px] transition-all">Batal</button>
                        <button onClick={handleSave} disabled={items.length === 0} className="flex-[2] md:flex-none px-16 py-5 bg-paper-blue hover:bg-paper-blueHover text-white font-black rounded-[2rem] shadow-2xl shadow-blue-500/30 transition-all active:scale-[0.98] disabled:opacity-30 uppercase tracking-[0.3em] text-[11px]">Finalisasi & Simpan Log</button>
                    </div>
                </div>
            </div>
        </div>
