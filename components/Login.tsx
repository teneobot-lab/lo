...
export const Login: React.FC<LoginProps> = ({ onLogin, notify }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F4F7FB] flex items-center justify-center p-4 transition-colors duration-500">
      <div className={`bg-white w-full max-w-md p-8 rounded-3xl shadow-xl shadow-indigo-100 border border-white transition-all duration-500 ease-in-out transform ${isSuccess ? 'scale-105 shadow-emerald-200' : ''}`}>
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center py-10 animate-in zoom-in duration-500">
            ...
            <h2 className="text-2xl font-bold text-dark">Login Successful!</h2>
          ...
          <div className={`transition-opacity duration-300 ${isLoading ? 'opacity-90' : 'opacity-100'}`}>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                    ...
                    <input 
                        type="text" 
                        disabled={isLoading}
                        className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              {error && <p className="text-rose-500 text-sm text-center font-medium bg-rose-50 py-2 rounded-lg animate-in fade-in slide-in-from-top-2">{error}</p>}
