...
export const Admin: React.FC<AdminProps> = ({ currentMediaUrl, onUpdateMedia }) => {
    ...
    const handleSaveConfig = () => {
        ...
        alert("Konfigurasi disimpan! Perubahan API akan memuat ulang halaman.");
  ...
  var sheet = ss.getSheetByName(payload.type) || ss.insertSheet(payload.type);
        ...
        <div className="space-y-8 max-w-7xl mx-auto pb-24">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 p-8 rounded-[2.5rem] shadow-soft border border-emerald-100 dark:border-emerald-900/30 overflow-hidden relative">
                        ...
                        <div className="flex items-center gap-4 mb-8">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-emerald-600 font-black uppercase tracking-widest">Cloud Database Bridge</p>
                            </div>
                        </div>

                        <div className="space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1">Apps Script Webhook URL</label>
                                <div className="flex gap-2">
                                    <input 
                                        value={sheetUrl} 
                                        onChange={e => setSheetUrl(e.target.value)} 
                                        placeholder="https://script.google.com/macros/s/.../exec" 
                                        className="flex-1 p-4 border border-emerald-100 dark:border-emerald-900/30 rounded-2xl text-sm bg-emerald-50/20 dark:bg-gray-900 dark:text-emerald-400 outline-none focus:ring-2 focus:ring-emerald-300 font-mono" 
                                    />
                            ...
                            <div className="p-6 bg-slate-50 dark:bg-gray-950 rounded-3xl border border-ice-100 dark:border-gray-800 space-y-4">
                                ...
                                <div className="group relative">
                                    <code className="block bg-black p-4 rounded-xl text-[10px] text-emerald-400 border border-slate-800 font-mono overflow-x-auto h-32">
                                        {appsScriptTemplate}
                                    </code>
                                    <button onClick={() => copyToClipboard(appsScriptTemplate)} className="absolute top-2 right-2 p-2 bg-slate-800 text-slate-400 rounded-lg opacity-0 group-hover:opacity-100 transition-all hover:text-white"><Copy size={12}/></button>
                                </div>
                            </div>
                        </div>
                    </div>
