      ...
      <div className="lg:col-span-2 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
        <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-paper border border-slate-200 dark:border-gray-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
              <div className="space-y-1.5">
                  ...
                  <div className="relative">
                      ...
                      <select value={warehouse} onChange={e => setWarehouse(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm font-bold focus:ring-2 focus:ring-paper-blue outline-none appearance-none cursor-pointer">
              ...
              {mode === 'transfer' && (
                  <div className="space-y-1.5">
                      ...
                      <div className="relative">
                          <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18}/>
                          <select value={targetWarehouse} onChange={e => setTargetWarehouse(e.target.value)} className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-gray-900 border border-slate-200 dark:border-gray-700 rounded-xl text-sm font-bold focus:ring-2 focus:ring-paper-blue outline-none appearance-none cursor-pointer">
              ...
              <div className="flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1 w-full relative">
                      ...
                      <div className="relative">
                          ...
                          <input type="text" className="w-full pl-12 pr-4 py-3 border border-slate-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-paper-blue outline-none transition-all" value={itemSearch} onChange={e => { setItemSearch(e.target.value); setShowDropdown(true); }} placeholder="Ketik SKU atau Nama Barang..." />
                      </div>
                  ...
                  <div className="w-full md:w-32">
                      ...
                      <input type="number" value={qty} onChange={e => setQty(e.target.value === '' ? '' : Number(e.target.value))} className="w-full p-3 border border-slate-200 dark:border-gray-700 rounded-xl text-sm font-black text-center text-paper-blue focus:ring-2 focus:ring-paper-blue outline-none" placeholder="0" />
                  </div>
          ...
          <div className="grid grid-cols-2 gap-6">
              <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1">Tanggal Transaksi</label><input type="date" value={customDate} onChange={e => setCustomDate(e.target.value)} className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none dark:bg-gray-900 dark:text-white" /></div>
              <div className="space-y-1"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block ml-1">Pihak Terkait (Supplier/Customer)</label><input value={supplier} onChange={e => setSupplier(e.target.value)} className="w-full p-3 border border-slate-200 rounded-xl text-sm outline-none dark:bg-gray-900 dark:text-white" placeholder="Nama..." /></div>
          </div>
        </div>
      </div>

      {/* Invoice Panel */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-paper border border-slate-200 dark:border-gray-700 flex flex-col h-full overflow-hidden">
          <div className="p-6 bg-slate-50 dark:bg-gray-700/50 border-b border-slate-200 flex justify-between items-center">
              ...
              <div className="px-3 py-1 bg-white rounded-full text-xs font-bold text-paper-blue border border-slate-100">{cart.length} Item</div>
          </div>
          <div className="flex-1 overflow-auto custom-scrollbar">
              <table className="w-full text-left">
                  <thead className="bg-slate-50 dark:bg-gray-800 border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-gray-700">
                      ...
                      {cart.length === 0 && <tr><td colSpan={3} className="p-12 text-center text-slate-300 italic text-sm">Belum ada barang di rincian.</td></tr>}
                  </tbody>
              </table>
          </div>
          <div className="p-6 bg-slate-50 dark:bg-gray-700 border-t border-slate-200">
...
const MenuButton = ({ title, sub, icon, color, onClick }: any) => {
    const colors: any = {
        emerald: "bg-emerald-50 text-emerald-500 border-emerald-100 hover:shadow-emerald-500/10",
        rose: "bg-rose-50 text-rose-500 border-rose-100 hover:shadow-rose-500/10",
        blue: "bg-blue-50 text-paper-blue border-blue-100 hover:shadow-blue-500/10"
    };
    return (
